...
func _on_add_anim_button_pressed():
	...
	if !%FramesWideLineEdit.text.is_valid_float() and float(%FramesWideLineEdit.text) <= 0:
		$AcceptDialog.dialog_text = "Invalid Frames Wide number"
		%AcceptDialog.popup_centered()
		return
	...
	new_part.vertical = %VerticalCheckButton.button_pressed
	new_part.start_index = int(%StartFrameLineEdit.text)
	new_part.end_index = int(%EndFrameLineEdit.text)
	new_part.frames_wide = int(%FramesWideLineEdit.text)
	...
	for anim_part in animation_parts:
		...
		for i in sprite_node_names.size():
			...
			if !anim_part.vertical:
				...
				for j_ in range(anim_part.start_index, anim_part.end_index + 1):
					var j = j_/anim_part.frames_wide + (j_%anim_part.frames_wide)*anim_part.frames_wide
					animation.track_insert_key(i, float(j_ - anim_part.start_index) / anim_part.fps, j)
